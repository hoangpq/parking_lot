...
impl RawRwLock {
    ...
    fn unlock_shared_slow(&self) {
        // At this point WRITER_BIT is set and READER_MASK is empty. We just
        // need to wake up a potentially sleeping pending writer.
        unsafe {
            // Using the 2nd key at addr + 1
            let addr = self as *const _ as usize + 1;
            let callback = |_| TOKEN_NORMAL;
    ...
    fn wait_for_readers(&self, timeout: Option<Instant>, prev_value: usize) -> bool {
        ...
        while state & READERS_MASK != 0 {
            ...
            unsafe {
                // Using the 2nd key at addr + 1
                let addr = self as *const _ as usize + 1;
                let validate = || self.state.load(Ordering::Relaxed) & READERS_MASK != 0;
                let before_sleep = || {};
                let timed_out = |_, _| {};
                match parking_lot_core::park(
                    ...
                    // since a previous writer timing-out could allow another
                    // reader to sneak in before we parked.
                    ParkResult::Unparked(_) => {
                        state = self.state.load(Ordering::Relaxed);
                        continue;
                    }

                    // The validation function failed, which means that there
                    // are no readers left.
                    ParkResult::Invalid => break,
        ...
        V: Fn(usize) -> bool,
    {
        ...
        loop {
            ...
            // If there are no parked threads, try spinning a few times. We only
            // do this if there are no active readers that a writer is waiting
            // for.
            if state & (PARKED_BIT | READERS_MASK) == 0 && spinwait.spin() {
                state = self.state.load(Ordering::Relaxed);
                continue;
            }
